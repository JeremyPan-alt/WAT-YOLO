        ...
        while True:
            ...
            for i, batch in pbar:
                ...
                with autocast(self.amp):
                    ...
                    # --------- 硬编码的超参（直接写死） ----------
                    dwt_lambda = 1e-4  # 正则权重（可按需修改）
                    dwt_warmup = 10  # 前 dwt_warmup 个 epoch 不启用正则

                    # --------- 计算 DWT 正则项（遍历 model.modules()） ----------
                    loss_reg = 0.0
                    for m in self.model.modules():
                        if hasattr(m, 'dwt_regularizer'):
                            try:
                                reg = m.dwt_regularizer()
                                # 把 reg 转为 tensor 并移动到 loss_det 相同 device/dtype
                                if not isinstance(reg, torch.Tensor):
                                    reg = torch.tensor(float(reg), device=loss_det.device, dtype=loss_det.dtype)
                                else:
                                    reg = reg.to(device=loss_det.device, dtype=loss_det.dtype)
                            except Exception:
                                # 出错时容错，避免训练中断
                                reg = torch.tensor(0.0, device=loss_det.device, dtype=loss_det.dtype)
                            loss_reg = loss_reg + reg

                    # --------- 根据 epoch 决定是否启用正则（warmup 控制） ----------
                    cur_epoch = getattr(self, 'epoch', None)
                    if cur_epoch is not None and cur_epoch < dwt_warmup:
                        effective_lambda = 0.0
                    else:
                        effective_lambda = dwt_lambda

                    # --------- 构造总损失并保留原有缩放逻辑 ----------
                    total_loss = loss_det + effective_lambda * loss_reg
                    self.loss = total_loss
                    if RANK != -1:
                        self.loss *= world_size

                    # --------- 保持原有 tloss 聚合逻辑，并把 dwt_reg 写入 loss_items 以便日志记录 ----------
                    try:
                        self.tloss = (
                            (self.tloss * i + self.loss_items) / (i + 1) if self.tloss is not None else self.loss_items
                        )
                        if isinstance(self.loss_items, dict):
                            self.loss_items['dwt_reg'] = float(loss_reg.item()) if isinstance(loss_reg,
                                                                                              torch.Tensor) else float(
                                loss_reg)
                        elif isinstance(self.loss_items, (list, tuple)):
                            temp = list(self.loss_items)
                            temp.append(
                                float(loss_reg.item()) if isinstance(loss_reg, torch.Tensor) else float(loss_reg))
                            self.loss_items = temp
                    except Exception:
                        # 不强求 loss_items 可变结构，出错则忽略记录
                        pass
                    # ---------- end 约束项引入 ----------
